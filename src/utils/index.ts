 ...
 * @param lyrics
 */
export function parseLyrics(lyrics: string): MusicLyricsType[][] {
	const lines = lyrics.split(';');
	let result = lines.map(line => {
		const words: MusicLyricsType[] = [];
		const regex = /\[(\d{2}):(\d{2}).(\d{3})\](.*?)(?=\[|$)/g;
		let match;
		while ((match = regex.exec(line)) !== null) {
			const time = parseFloat((parseInt(match[1]) * 60 + parseInt(match[2]) + parseInt(match[3]) / 1000).toFixed(3));
			const text = match[4] ?? '';
			words.push({
				time,
				word: text
